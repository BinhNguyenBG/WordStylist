...
def main():
    ...
    unet.load_state_dict(torch.load(f'{args.models_path}/ckpt.pt', map_location=args.device))
    optimizer.load_state_dict(torch.load(f'{args.models_path}/optim.pt', map_location=args.device))
    ...
    ema_model.load_state_dict(torch.load(f'{args.models_path}/ema_ckpt.pt', map_location=args.device))
    ...
    if not args.single_image:
        ema_sampled_images = diffusion.sample(ema_model, vae, n=len(labels), x_text=words, labels=labels, args=args)
        sampled_ema = save_images(ema_sampled_images, os.path.join(args.save_path, 'images', f"{words}.jpg"), args)
    else:
        if args.interpolation == True:
            ema_sampled_images = diffusion.sampling(ema_model, vae, n=len(labels), x_text=words, labels=labels, args=args)
            sampled_ema = save_single_images(ema_sampled_images, os.path.join(args.save_path, 'images', f"{words}_{args.mix_rate}.png"), args)
        else:
            #for i in range(10):
            ema_sampled_images = diffusion.sampling(ema_model, vae, n=len(labels), x_text=words, labels=labels, args=args)
            sampled_ema = save_single_images(ema_sampled_images, os.path.join(args.save_path, 'images', f"{words}_{s[0]}.png"), args)
